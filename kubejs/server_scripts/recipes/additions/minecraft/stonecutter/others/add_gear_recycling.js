...
ServerEvents.recipes(event => {
    ...
    iron_armor_items.forEach(item => {
        event.stonecutting('2x minecraft:iron_ingot', item).id(`kubejs:stonecutting/recycle_${item.replace('minecraft:', '')}`);
    });

    // SỬA LỖI TẠI ĐÂY: Dùng .concat() để nối mảng thay vì cú pháp '...'
    iron_tool_items.concat(chainmail_armor_items).forEach(item => {
        event.stonecutting('minecraft:iron_ingot', item).id(`kubejs:stonecutting/recycle_${item.replace('minecraft:', '')}`);
    });

    gold_gear_items.forEach(item => {
        event.stonecutting('minecraft:gold_ingot', item).id(`kubejs:stonecutting/recycle_${item.replace('minecraft:', '')}`);
    });

    diamond_armor_items.forEach(item => {
        event.stonecutting('2x minecraft:diamond', item).id(`kubejs:stonecutting/recycle_${item.replace('minecraft:', '')}`);
    });

    diamond_tool_items.forEach(item => {
        event.stonecutting('minecraft:diamond', item).id(`kubejs:stonecutting/recycle_${item.replace('minecraft:', '')}`);
    });
